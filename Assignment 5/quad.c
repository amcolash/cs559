...
void usage() {
  fprintf(stderr, "usage: quad.o <outfile> <debug?> <p0.x> <p0.y> <p1.x> <p1.y> <p2.x> <p2.y>\n");
  exit(1);
}
...
int main(int argc, char *argv[]) {
  ...
  sprintf(buffer, "<path d='M%.2f,%.2f Q%.25f,%.2f %.2f,%.2f' fill='none' stroke='black' stroke-width='2'/>", p0.x, p0.y, p1.x, p1.y, p2.x, p2.y);
  fputs(buffer, outFile);

  sprintf(buffer, "<circle cx='%.2f' cy='%.2f' r='8' fill='blue'/>", p0.x, p0.y);
  fputs(buffer, outFile);
  sprintf(buffer, "<circle cx='%.2f' cy='%.2f' r='8' fill='blue'/>", p2.x, p2.y);
  fputs(buffer, outFile);
